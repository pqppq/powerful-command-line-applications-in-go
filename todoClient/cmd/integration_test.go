...
func randomTaskName(t *testing.T) string {
	t.Helper()
	const chars = "abcdefgijklmnopqrstuvwxyzABCDEFGIJKLMNOPQRSTUVWXYZ0123456789"
...
func TestIntegration(t *testing.T) {
	...
	t.Run("AddTask", func(t *testing.T) {
		...
		if expOut != out.String() {
			t.Errorf("Expected output %q, got %q instead", expOut, out.String())
		}
	})
	t.Run("ListTasks", func(t *testing.T) {
		...
		if taskCompleteStatus != "-" {
			t.Errorf("Expected status %q, got %q instead", "-", taskCompleteStatus)
		}
		taskId = strings.Fields(outList)[1]
	})

	vRes := t.Run("ViewTasks", func(t *testing.T) {
		var out bytes.Buffer
		...
		if !strings.Contains(viewOut[0], task) {
			t.Fatalf("Expected task %q, got %q instead", task, viewOut[0])
		}
		if !strings.Contains(viewOut[1], today) {
			t.Fatalf("Expected creation day/month %q, got %q instead", today, viewOut[1])
		}
		if !strings.Contains(viewOut[2], "No") {
			t.Fatalf("Expected completed status %q, got %q instead", "No", viewOut[2])
		}
	})
	if !vRes {
